...
Contents include:
* BlockChain, Block (New) (Rust Powered)
* Directed Acyclic Graph (DAG) (New) (Rust Powered)
* String (upgraded(str))
* List (upgraded(list))
* HashMap (upgraded(dict))
* AutoHashMap( New)
* Stack (New)
* Queue (New)
* Priority Queue (New)
* SingleLinkNode (New) (Singly Linked Node)
* LinkedListOne (New) (Single Linked List)
* DoubleLinkNode (New) (Doubly Linked Node)
* LinkedListTwo (New) (Doubly Linked List)
...
* Property class for property creation automation.
* Possibility class for extending return statements.
* Random class for choosing a random value from a pool based on several in-built + custom algorithms.
* 'terminal' module for terminal coloring and terminal size queries.
* A re-defined fully customizable ProgressBar class along with several in-built widgets and ways to create custom widget. This bar supports wrapping shell (bash) codes/scripts, python codes/scripts, and powershell codes/scripts (later: coming soon).
...
Contents include:
* Scroller class to create scrollable frames/container that dynamically handles scrolling even if it's parent or child tries to race for <MouseWheel> binding.
* Toolbar class to create a toolbar widget for your GUI application.
* Page class to create pages for your application and easily handle their order and view conditions.
* TkinterBuilder class for creating tkinter widgets in one line. (under works)
...
Contents include:
* PathWays class for creating cases.
* Definition class for defining arguments.
